...
class CKA:
    ...
    def scores(self, model, x_train=None, y_train=None, allowed_layers=[], n_samples=None):
        output = []
        network_cka_scores = []  # Nova lista para CKA da rede
        ...
        for i in range(len(allowed_layers)):
            ...
            for h in range(n_heads):
                ...
                network_cka_scores.append(1 - score)  # Armazena CKA da rede neste estado

            output.append((allowed_layers[i], scores))

        return output, network_cka_scores  # Retorna ambos os valores
