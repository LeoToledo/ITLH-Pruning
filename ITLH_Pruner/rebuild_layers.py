...
def get_layers_to_prune(model, scores, p):
    """
    Determina quais camadas devem ser podadas
    
    Args:
        model: modelo original
        scores: pontuações de importância das camadas
        p: proporção ou número de camadas a serem podadas
        
    Returns:
        blocks_tmp: número de blocos após a poda
        mask: máscara binária indicando quais camadas manter
        allowed_layers: lista de camadas permitidas para poda
    """
    blocks = count_blocks(model)
    allowed_layers = [x[0] for x in scores]
    scores_values = [x[1] for x in scores]
    blocks_tmp, mask = new_blocks(blocks, np.copy(scores_values), allowed_layers, p)
    
    return blocks_tmp, mask, allowed_layers

def rebuild_network(model, blocks_tmp, mask, allowed_layers):
    """
    Reconstrói o modelo removendo as camadas especificadas
