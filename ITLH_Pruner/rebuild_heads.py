...
def get_heads_to_prune(model, scores, p):
    """
    Determina quais heads devem ser podados em cada camada
    
    Args:
        model: modelo original
        scores: pontuações de importância dos heads
        p: proporção ou número de heads a serem podados
        
    Returns:
        layer_heads: lista de tuplas (índice_camada, índices_heads_para_podar)
    """
...
def rebuild_network(model, layer_heads):
    """
    Reconstrói o modelo removendo os heads especificados
